const web = "https://citzenkan.000webhostapp.com/";
...
async function signupHosp(form) {
  ...
  myHeaders.append("login", "true");
  myHeaders.append("reqType", "signup");
  var requestOptions = {
    method: "POST",
    headers: myHeaders,
    body: user,
  ...
  const res = await fetch(HOST + "signup.php", requestOptions);
  const data = await res.json();
  console.log(data);
  if (data.ok) {
    localStorage.setItem("token", data.token);
